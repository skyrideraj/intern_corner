...
class RuralOpportunity extends Opportunity
{
...
static function upVote($opportunity_id){
		...
		$db->query("SELECT * FROM rural_opportunity WHERE user_name='$username' AND opportunity_id=$opportunity_id");
		...
		//200=OKAY
		return array('status_code'=>200,'tot_votes'=>RuralOpportunity::calculateVotes($opportunity_id));
		}
		else{
...
static function calculateVotes($opportunity_id){
 ...
 function addOpportunity(){
		...
		if($account_type==2){
			...
			if($num_results==0){
				...
				//retrieve his last limit posts and then check their timestamps
					$db->query("SELECT * FROM rural_opportunity WHERE user_name = '$username' LIMIT $limit_of_day");
		...
		if($allowed_to_post==1){
			...
			$db->query("INSERT INTO opportunity (opportunity_id,opportunity_type,timestamp) VALUES(NULL,3,NULL)");
			...
			$db->query("INSERT INTO rural_opportunity(`opportunity_id`, `name`, `user_name`, `description`, `deadline_for_application`, `start_date`, `end_date`, `stipend`, `organization`, `location`) VALUES($id,'$this->title','$username','$this->description','$this->deadline','$this->start_date','$this->end_date','$this->stipend','$this->organization','$this->location')");
			
			return array('status_code'=>200,'detail'=>"rural opportunity posted");
    ...
    static function extractOpportunity($page_no){
		...
		// echo $no_of_posts;
		$db->query("SELECT * FROM rural_opportunity ORDER BY 'timestamp' DESC LIMIT $no_of_posts");
		$result = $db->fetch_assoc_all();
		...
		for($i=$low;($i<$high)&&($i<$rows_ret);$i++){
			...
			$total_votes = RuralOpportunity::calculateVotes($result[$i]['opportunity_id']);
			
			// $tags = RuralOpportunity::extractTags($result[$i]['opportunity_id']);
			
			// construct object accordingly 
			$opp = new RuralOpportunity($result[$i]['user_name'],$result[$i]['opportunity_id'],$result[$i]['name'],$result[$i]['description'],$result[$i]['deadline_for_application'],$result[$i]['start_date'],$result[$i]['end_date'],$result[$i]['stipend'],$result[$i]['organization'],$result[$i]['location'],NULL,$result[$i]['timestamp'],NULL);
...
//$var = RuralOpportunity::extractOpportunity(0);
//print_r($var);
