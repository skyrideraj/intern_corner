...
class Post
{
	...
	function __construct($post_id, $title, $description, $answers, $comments, $username, $total_votes, $tags, $timestamp, $anonymous, $no_of_close_requests)
	{
		# code...
...
<<<<<<< HEAD
			...
			for($i=0;$i<sizeof($tags_arr);$i++){
				...
				if($indi_tag!=" "&&$indi_tag!=""){
				$db->query("SELECT * FROM tag_table WHERE tag_name='$indi_tag'");
				if($db->returned_rows==0){
					//insert in tag table
					$db->query("INSERT INTO tag_table VALUES(NULL,'$indi_tag')");
				}
				//assosiate post with tag
				$db->query("SELECT * FROM tag_table WHERE tag_name='$indi_tag'");
				$results = $db->fetch_assoc_all();
				$tag_id = $results[0]['tag_id'];
				$db->query("INSERT INTO post_tags VALUES($post_id,$tag_id)");
...
<<<<<<< HEAD
	function sendMailToTagSubscribers($post_id){
		...
		for($i=0;$i<sizeof($tags_arr);$i++){
			...
			$tag_id = $results[0]['tag_id'];
			//extract its subscribers
			$db->query("SELECT * FROM $table_name WHERE tag_id=$tag_id");
			$results2 = $db->fetch_assoc_all();
				for($i=0;$i<sizeof($results2);$i++){
					$username = $results2[$i]['user_name'];
					$db->query("SELECT * FROM user WHERE user_name='$username'");
					$results3 = $db->fetch_assoc_all();
					$email = $results3[0]['email'];
					Post::sendMail($email,$indi_tag);
	...
	static function sendMail($email,$tag_name){
		//mail($email,"New Post","A new post has been added to $tag_name");
	}
...
>>>>>>> a84ff92c4e6e02a26c77a9ea0d98ea4879eaddf7
	...
	static function upVote($post_id){
		...
		$rest = $db->fetch_assoc_all();
		$author = $rest[0]['user_name'];
		//check if student
		$db->query("SELECT * FROM user WHERE user_name='$author'");
		$rest = $db->fetch_assoc_all();
		$type = $rest[0]['account_type'];
		if($type==2){
			$db->query("SELECT * FROM student WHERE user_name='$author'");
			$rest7 = $db->fetch_assoc_all();
			
			$reputation_author = $rest7[0]['reputation'];
			$db->query("UPDATE student SET reputation=($reputation_author+5)");
		}
...
static function subscribeTag($tag_id){
		@session_start();
		$username = $_SESSION['user']->getUsername();
		$db = (new Database())->connectToDatabase();
		//has user already subscribed?
		$db->query("SELECT * FROM tag_subscribed WHERE user_name='$username' AND tag_id=$tag_id");
		if($db->returned_rows==0){
			$db->insert('tag_subscribed',array('user_name'=>$username,'tag_id'=>$tag_id));
				// OK
			return array('status_code'=>200);
			...
			//already subscribed
			//bad request
			$db->query("DELETE FROM tag_subscribed WHERE user_name='$username' AND tag_id=$tag_id");
	...
	static function delete_post($post_id){
		...
		//print_r("$results[0]");
		$post_author = $results[0]['user_name'];
		...
		if($reputation>$threshold_to_delete){
			...
			if($num_results>=3){
				...
				return array('status_code'=>203,'detail'=>'request sent');
			}
