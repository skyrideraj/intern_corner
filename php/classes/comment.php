...
$db = (new Database())->connectToDatabase();
	$db->query("SELECT * from post_comments where comment_id='$id' and user_name='$username'");
	$rows=$db->returned_rows;
	if($rows>0){
		$db->query("DELETE from post_comments where comment_id='$id' and user_name='$username'");
		return array('status_code'=>200,'detail'=>'Comment deleted');
		}
	else{
