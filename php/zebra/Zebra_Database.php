 ...
 *  provides methods for interacting with MySQL databases that are more powerful and intuitive than PHP's default ones.
 *
 *  It supports transactions and provides ways for caching query results either by saving cached data on the disk, or by
 *  using memcache.
 *
 ...
 *  @version    2.7.3 (last revision: July 16, 2012)
...
class Zebra_Database
{
     ...
     *                      <br>
     *                      <i>For using memcache as caching method, PHP version must be 4.3.3+, must be compiled with the
     ...
     *
     *  <i>If caching method is set to "memcache", {@link memcache_host}, {@link memcache_port} and, optionally
     *  {@link memcache_compressed}, must be set *prior* to calling the {@link connect()} method! Failing to do so
     *  will disable caching.</i>
     *
     *  <code>
    ...
    function Zebra_Database()
    {
        ...
        $this->database = $this->link_identifier = $this->memcache = $this->memcache_host = $this->memcache_port = false;
     ...
     *  @return void
     */
    function connect($host, $user, $password, $database, $is_new = false, $connect = false)
    {
        ...
        if (extension_loaded('memcache') && $this->caching_method == 'memcache')
            ...
            // supress the warning telling the developer to use memcache for caching query results
    ...
    function escape($string)
    {
        ...
        if ($this->_connected()) {
            ...
            return mysql_real_escape_string($string, $this->link_identifier);
    ...
    function fetch_assoc($resource = '')
    {
        ...
        if ($this->_connected()) {
            ...
            if (is_resource($resource)) return mysql_fetch_assoc($resource);
    ...
    function fetch_obj($resource = '')
    {
        ...
        if ($this->_connected()) {
            ...
            // if $resource is a valid resource, fetch and return next row from the result set
            if (is_resource($resource)) return mysql_fetch_object($resource);
    ...
    function insert_id()
    {
        ...
        if ($this->_connected())
            ...
            if (isset($this->last_result)) return mysql_insert_id($this->link_identifier);
        ...
        if ($this->_connected()) {
            ...
            elseif ($replacements != '' && is_array($replacements)) {
                ...
                // if the number of items to replace is the same as the number of items specified in $replacements
                    ...
                    foreach ($replacements as $key => $replacement) {
                        ...
                        // otherwise, mysql_real_escape_string the items in replacements
            ...
            if ($this->last_result !== false) {
                ...
                if (is_resource($this->last_result) || $this->last_result === true) {
                    ...
                    if (is_resource($this->last_result)) $is_select = true;
                    ...
                    } else $this->affected_rows = @mysql_affected_rows($this->link_identifier);
                    ...
                    if ($is_select && $cache !== false) {
                        ...
                        while ($row = mysql_fetch_assoc($this->last_result)) $cache_data[] = $row;
                    ...
                    if ($is_select) {
                        ...
                        if ($is_select && is_resource($this->last_result)) {
                            ...
                            if ($explain_resource)
                                ...
                                while ($row = mysql_fetch_assoc($explain_resource)) $explain[] = $row;
    ...
    function seek($row, $resource = '')
    {
        ...
        if ($this->_connected()) {
            ...
            if (is_resource($resource)) {
                ...
                // we have to mute error reporting because if the result set is empty (mysql_num_rows() == 0),
                // a seek to 0 will fail with a E_WARNING!
                if (@mysql_data_seek($resource, $row)) return true;
     ...
     *                                  the actual results of the query!</i>
     */
    function select($columns, $table, $where = '', $replacements = '', $order = '', $limit = '', $cache = false, $highlight = false, $calc_rows = false)
    {
    ...
    function _connected()
    {
        ...
        if (!$this->memcache && $this->memcache_host !== false && $this->memcache_port !== false) {
            ...
            // if memcahe extension is not installed
            } else
